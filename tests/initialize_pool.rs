...
fn initialize_pool() {
    ...
    let initialize_tx = StakingInstruction::Initialize {
        rewards_per_token: 42,
    };
    let mut instruction_data: Vec<u8> = vec![];
    // Serialize instruction into bytes that would be given as instruction_data to entrypoint!
    initialize_tx.serialize(&mut instruction_data).unwrap();
